...
6. <span style="color: red;">If</span> $\mathsf{\textcolor{red}{skip\_aud\_check\_val}}$ <span style="color: red;">is set
   - <span style="color: red;"> *Then:* assert $\mathsf{aud\\_val}$ is the empty string.
   - <span style="color: red;"> *Else:* Are we in normal mode (i.e., we are not in recovery mode $\Leftrightarrow \mathsf{override\\_aud\\_val} = \bot$)
       + <span style="color: red;"> *Then:* check the managing application’s ID in the JWT: assert $\mathsf{aud\\_val}\stackrel{?}{=}\mathsf{jwt}[\texttt{"aud"}]$
