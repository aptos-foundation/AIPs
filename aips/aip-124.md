...
## Summary
A scheduled transaction is a user-submitted transaction that specifies another transaction to be executed by a specific trigger, e.g., a time-based or event-based trigger. The system executes the specified transaction in a suitable future block.  
...
#### Failure Retries
If a scheduled transaction fails during execution, no retry attempts are made, and the user must manually reschedule the transaction.
