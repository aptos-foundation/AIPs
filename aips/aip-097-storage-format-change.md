...
- `EventByKey`, `EventByVersion`: This two indices are used to handle `/accounts/{address}/events/{event_handle}/{field_name}` and `/accounts/{address}/events/{creation_number}` APIs. Given Aptos is moving to adopt [Module Event](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-044-module-events.md), which is a completely different model, these APIs would not make sense anymore, and are considered as deprecated. The only reason they are kept for now is to query events under old event framework.
