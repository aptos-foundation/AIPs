 ...
 **The system supports a fixed maximum number of blocks per epoch, determined by the trusted setup size.**
 - We are running a trusted setup with a much larger number of possible blocks per epoch than is currently possible on mainnet. So the chances of running into this limit are very low.
