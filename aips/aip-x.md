...
**Flow.** The scheme is integrated into consensus, allowing the validators
to agree on a set of ciphertexts, and to decrypt this set immediately after
consensus is reached on the block. Specifically:
* After receiving a block proposal from the leader, each validator first
  invokes `verify_ciphertext` on each ciphertext uses the
  `digest` method on the set of ciphertexts in the block to compute
  a constant-sized `Digest` and an `EvalProofsPromise`. `digest`
  additionally requires as input a `DigestKey`, which is a file generated by the
  trusted setup ceremony and loaded by each validator.
