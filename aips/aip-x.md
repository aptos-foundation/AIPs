...
title: SLH-DSA-SHA2-128s signatures for post-quantum Aptos accounts
...
# AIP-X - SLH-DSA-SHA2-128s signatures for post-quantum Aptos accounts
...
Both its classical and quantum security rely solely on the security of the SHA2-256[^ABBplus22] hash function.
 ...
 > [!NOTE]
 > The v3.1 spec[^ABBplus22] clarifies in Section 7.2.2 that the $n=16$ SHA2 variants uses SHA2-256.
...
For the purpose of this AIP, _"conservative"_ means:
 
 - the security assumptions behind the CRQC scheme are abundantly cautious: i.e., breaking them quantumly or classically would be an immense technological surprise
...
In particular, the SLH-DSA family is appealing because it only relies on hash functions and is thus clearly not likely to be broken clasically.
(Furthermore, the relied-upon hash functions are already trusted today in the Aptos ecosystem.)
This stands in contrast with other, more "aggresive", PQ schemes, which rely on exotic assumptions that may not even be clasically-secure (e.g., see the [Rainbow discussion](#alternative-solutions)).
As a result, such aggressive schemes need to be combined with a clasically-secure scheme to mitigate risks, which increases complexity.
...
### Out of scope
 ...
 - Aggresively-optimizing the choice of the signature scheme to maximize performance. This will be done later by adding more PQ schemes.
...
If we accept, deploy and enable this proposal, it would impact almost all of the Aptos ecosystem:
 ...
 - (There may be other components there were missed.)
...
Such attacks have happened in the past on candiate PQ schemes: e.g., _Rainbow_, based on multivariate cryptography, was a NIST finalist yet it was completely broken on a commodity laptop in 2022[^Beul22].
...
Notes:
 ...
 - We unit test for:
	+ correctness (i.e., honestly-produced signatures pass verifications)
...
PRs:
 ...
 - [ ] [Add SLH-DSA-SHA2-128s (FIPS 205) to `aptos-crypto` (#18293)](https://github.com/aptos-labs/aptos-core/pull/18293)
...
Testing plan:
 ...
 - Move e2e VM test ensuring that TXN signatures verify
    + Or a slower smoke test
 - _Maybe:_ e2e test using the SDK and/or the CLI

### Benchmarks 

Signatures sizes are 7,856 bytes.
...
Secret key sizes are 48 bytes.
 ...
 > [!WARNING]
 > **Hopefully, this is wrong, but:** the FIPS standard suggests SKs are 32 bytes only by (recklessly?) assuming that the PK is available and not lost.
...
In particular, signature verification times are **6x slower than Ed25519** (these are on a Macbook Pro M1 Max but x86_64 times should be similar):
```
...
```
 ...
 > [!NOTE]
 > For more PQ signature schemes bechmarks see this public GitHub repo by Project 11[^benching-pq].

## Risks and Drawbacks

What are the hazards? What can go wrong?
 
First, TXN signatures will be larger: 7,856 bytes.
This is 123 $\times$ larger than Ed25519 (64 bytes) and 27 $\times$ larger than a `KeylessSignature` ($\approx$ 294 bytes).
...
Since this family only relies on hash functions, it remains clasically-secure under the same assumptions used throughout the Aptos ecosystem.
...
The SHA2 variant of SLH-DSA was chosen due to its higher verification speed when compared to the SHAKE variants (e.g., see Macbook Pro M1 Max benchmarks[^rust-crypto-benches]).
 ...
 > [!WARNING]
 > They were deemed faster on ARM, but should confirm on x86_64.
...
One possibility is that a CRQC does _not_ materialize in the next 5 years **but yet** a lot of concerned Aptos users have nonetheless chosen to use this scheme.
...
Milestones:
 ...
 - [ ] Feature-gated Aptos VM signature verification logic for these new PQ accounts
...
### Q1: Should we used hedged (randomized) signing or deterministic signing (currently-enabled)?
...
It is unclear what would be lost by enabling it, beyond some level of conceputal simplicity.

### Q2: Should BHT-style[^BHT97] quantum attacks on hash functions simply be dismissed?

They do not seem to be considered in the NIST submission[^ABBplus22].
Perhaps their impracticality may be the reason?[^Bern09]

### Q3: To what extent are RowHammer attacks on SLH-DSA concerning?
