...
Our (temporary) decisions:
 ...
 - We set:
    + the 16-byte SK seed and 16-byte PRF seed to the 32-byte secret key outputted by BIP-32's[^bip-32] `CKDPriv` derivation function
...
In our TypeScript SDK, the implementation for deriving an SLH-DSA private key given a mnemonic-derived `seed` and a derivation path `path` looks like this:
```typescript
    ...
    threeSeeds.set(finalSecretKey, 0); 

    // Last 16 bytes from the derived chain code
    threeSeeds.set(finalChainCode.slice(0, 16), 32);
```
...
### Q1: How to implement HD wallet for this scheme?

The goal would be to maximize the chance of compatibility with future implementations in hardware wallets.

The difficulty is that we'd be proverbially "shooting in the dark:" it's difficult to fathom what hardware wallet implementors like Ledger or Trezor will choose to do.
