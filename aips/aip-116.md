...
The interface for the signing request sent to the wallet may have a variant of the `AptosSignInInput` interface that omits the `domain` field to prevent the creation of application-specific phishing messages.
...
| ----------------- | ------ | ------------------------------------------------------------------------------------------------------------------ |
| account.address   | string | The address of the user that is signing in. This field MUST match the `address` field used to construct `message`. |
| account.publicKey | bytes  | The public key of the user that is signing in.                                                                     |
| message           | string | The constructed SIWA message using the standardized message format.                                                |
| signature         | bytes  | The signature of the signing message.                                                                              |
| signatureType     | string | The digital signature scheme used to sign (e.g. 'ed25519', 'single_key', 'multi_key', 'multi_ed25519', etc.).      |
...
2. The application will make a call to the backend to construct an `AptosSignInInput` object. This object can be partially filled, only requiring the fields that have been marked mandatory in the [`AptosSignInInput`](#aptossignininput) table above. This partially filled `AptosSignInInput` object should be stored to be verified later in step 6.
...
4. The wallet will construct a SIWA message from the `AptosSignInInput`, binding required fields that are not present in the partially filled `AptosSignInInput` object. Once the SIWA message is constructed, the SIWA message will go into the [Domain Separated Signing Algorithm](#domain-separated-signing-algorithm) to create a SIWA signing message. The prompt will display the SIWA message in a UI that can easily be read by the user.
5. When the user approves to sign the message, the wallet will use the Aptos account will be used to sign the SIWA signing message and return an `AptosSignInOutput` object to the application.
6. Once obtained, the application should send the `AptosSignInOutput` object to the backend, where it will verify the `signature` and `message` using [Signature and Message Verification](#signature-and-message-verification). For the message verification, the application should use the partially filled `AptosSignInInput` object from step 2 as the `expected` fields.
7. If it verifies successfully, the backend will proceed to authenticate the user (e.g. issue a JWT token, session cookies, etc) and give them access to the resource.
...
The proposed plain text representation of an `AptosSignInInput` object will be an adaptation of the standards specified from [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) and [CAIP-122](https://chainagnostic.org/CAIPs/caip-122). If certains fields are not present in the `AptosSignInInput`, omit them from the SIWA message.

If the following fields are not present in the `AptosSignInInput`, they must be bound by the wallet: `address`, `chainId`, `domain`, `uri`, and `version`.
...
A plain text construction of a SIWA message (as described [here](#siwa-message-construction)) must be prefixed with a `SIGN_IN_WITH_APTOS::` hash before signing to ensure domain separation across other signatures. The SIWA signing messages must conform to the following expression:
...
1. **Signature verification** - Cryptographically ensures the authenticity of a signature against the `message`.
2. **SIWA Message verification** - Ensures the content of the `AptosSignInOutput` message adheres to the expected fields.

#### Signature Verification

Signature verification must consist of three parts:

- `publicKey` - The public key of the user that is signing in.
- `signature` - The signature to verify.
- `message` - The SIWA message to verify.

To verify the `signature`, the `publicKey` should be used to cryptographically verify the `signature` against the `message`. This verification should be stateless.

#### Message Verification

The message verification process must consist of the following parts:
...
- `expected` - The `AptosSignInInput` fields that are expected to be present in the `message`.
- `message` - The SIWA message to verify.

The verification process must perform the following steps:

1. Parse the `message` into an `AptosSignInInput` object.
2. Compare the `expected` fields against the parsed `AptosSignInInput` object. If fields are present in the `expected` object, they must match the corresponding fields in the parsed `AptosSignInInput` object.
3. Derive the authentication key from the `publicKey` and check if it is equal to the `address` field in the parsed `AptosSignInInput` object. If not equal, check that, on-chain, the `authentication_key` field of the Aptos `Account` resource for the `address` is equal to the authentication key of the `publicKey`.
...
Wallets may inject additional information into the `resources` field for **wallet specified resources** and in any **wallet specified formats**.
...
/**
 * Create a SIWA Message from the `AptosSignInInput` with required fields following the ABNF format defined in the Sign in with Aptos
 * specifications.
 *
 * @param input The `AptosSignInInput` with required fields to create the SIWA message from.
 *
 * @returns The SIWA message.
 */
function createSignInMessageText(
 ...
 */
function parseSignInMessageText(
  text: string
...
async function verifySignInMessage(
  ...
  const { publicKey, expected, message } = params;

  // 1. Parse the message into `AptosSignInInput & AptosSignInRequiredFields`
  const parsed = parseSignInMessageText(message);
  if (!parsed.valid) return parsed;

  // 2. Check if `PublicKey`'s authentication key equals the address in the parsed message
  ...
  if (!AccountAddress.from(parsed.data.address).equals(originalAddress)) {
    return { valid: false, errors: ["invalid_auth_key"] };
  }

  // 3. Check the fields from the parsed message against the expected fields
  const errors: VerificationComparisonError[] = [];
  if (expected.address && expected.address !== parsed.data.address)
    errors.push("message_address_mismatch");
  if (expected.statement !== parsed.data.statement)
    errors.push("message_statement_mismatch");
  if (expected.uri && expected.uri !== parsed.data.uri)
    errors.push("message_uri_mismatch");

  // ... additional field checks

  // 4. If there are no errors, return the parsed fields
  return errors.length === 0
    ? { valid: true, data: parsed.data }
    : { valid: false, errors };
}

async function verifySignInSignature(params: {
  publicKey: PublicKey;
  signature: Signature;
  message: string;
}): VerificationResult<AptosSignInInput & AptosSignInRequiredFields> {
  const { publicKey, signature, message } = params;

  // 1. Parse the message into `AptosSignInInput & AptosSignInRequiredFields`
  const parsed = parseSignInMessageText(message);
  if (!parsed.valid) return parsed;

  // 2. Verify the signature using the `publicKey`
  // Note: The implementation of `verify` is handled by the `@aptos-labs/ts-sdk` package
  const isSignatureValid = publicKey.verifySignature({
    message: generateSignInSigningMessage(message),
    signature,
  });
...
export default function SignInPrompt({
  ...
  const [verification, setVerification] = useState<VerificationResult<AptosSignInInput & AptosSignInRequiredFields>>();

  useEffect(() => {
    verifySignInMessage({
      input: createSignInMessageText({ ...getBoundFields(), ...input }),
...
1. The application's backend should provide an endpoint that constructs a `AptosSignInInput`. These `AptosSignInInput` fields should be stored at the beginning of the flow so that it can be used to verify the wallet’s `AptosSignInOutput` at the end of the flow.
...
3. The application should also create an endpoint to verify the `AptosSignInInput` constructed at the beginning of the flow to the `AptosSignInOutput` from the wallet. The verification typically requires the following from the `AptosSignInOutput`: signature, signature type, plain text message, and account public key. We use the `deserializeSignInPublicKey`, `deserializeSignInSignature`, `verifySignInSignature`, and `verifySignInMessage` utilities from `@aptos-labs/siwa` to handle the verification logic.
...
app.post("/auth/siwa/verify", (c) => {
  ...
  const messageVerification = verifySignInMessage({
    publicKey: deserializeSignInPublicKey(type, publicKey),
    expected: { ...input, domain: FRONTEND_URL },
    message,
  });
