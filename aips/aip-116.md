...
- **Enhanced Security:** By enabling wallets to validate and flag potentially malicious messages, SIWA eliminates the risk of phishing attacks and other security vulnerabilities that are present in the `connect` + `signMessage` flow.
...
The application is responsible for generating a partially filled `AptosSignInInput` object and storing it in the backend. This object is then sent to the wallet. The wallet completes the `AptosSignInInput` object with bound fields and constructs a SIWA message which is signed upon user approval. An `AptosSignInOutput` object is returned to the application and sent to the backend for verification using the originally stored `AptosSignInInput`. If verified, the user is authenticated and granted access to the application.
...
To verify the `signature`, the `publicKey` should be used to cryptographically verify the `signature` against the `message` and outputs `true` or `false` to indicate success or failure. This verification should be stateless.
...
- `expected` - The `AptosSignInInput` object that are expected to be present and match the `input` object.
- `input` - The `AptosSignInInput` object with bound fields that was used to construct the `input`.

The verification process must perform the following steps:

1. Derive the authentication key from the `publicKey` and check if it is equal to the `address` field in the parsed `AptosSignInInput` object. If not equal, check that, on-chain, the `authentication_key` field of the Aptos `Account` resource for the `address` is equal to the authentication key of the `publicKey`.
...
5. If all the above checks pass, return the `true`, otherwise return the `false`.
...
TODO: Point to reference implementations on Github

### Wallet Implementation

The wallet is responsible for taking `AptosSignInInput` object defined by an application and responding with an `AptosSignInOutput`. This section assumes that you have an understanding of the [AIP-62](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-62.md).
...
3. The wallet should also make sure to check that if `AptosSignInInput` has pre-filled bound fields, that they are equal to the bound fields the wallet derives from its trusted data source. Below is an example of how the fields can be checked and displayed to the user. **If there are fields that do not match, the wallet should NOT allow the user to approve the signing request.**
...
4. Once the user approves, the wallet should construct an `AptosSignInOutput` using the `AptosSignInInput` with bound fields. When signing, it is important that the user signs the signing message derived from the [`Domain Separated Signing Algorithm`](#domain-separated-signing-algorithm) and the [`SIWA message`](#siwa-message-construction).
...
1. The application's backend should provide an endpoint that constructs a `AptosSignInInput`. The `AptosSignInInput` object should be stored so that it can be used to verify the `AptosSignInOutput` at the end of the flow.
...
2. On the frontend, the application should call the newly added endpoint (in this case `/auth/siwa`) and send a `signIn` signing request to the wallet with the `AptosSignInInput` from that endpoint.
