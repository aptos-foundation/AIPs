...
- **trains the user to sign malicious messages —** Users are being trained to sign messages without understanding its contents. The long-term implications of this practice can lead to users becoming more susceptible to phishing attacks.
...
| message           | string | The constructed message using the standardized message format.                                                     |
...
3. Ensure that the `publicKey`'s authentication key's originating address is equal to the `address` field in the parsed `AptosSignInInput` object.
4. If there is no originating address, ensure that the `publicKey`'s authentication key is equal to the `address` field in the parsed `AptosSignInInput` object.
